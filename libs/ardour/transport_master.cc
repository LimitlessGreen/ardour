...
#define PLUSMINUS(A) (((A) < 0) ? "-" : (((A) > 0) ? "+" : "\xC2\xB1"))
...
TransportMaster::format_delta_time (sampleoffset_t delta) const
{
	...
	if (_session) {
		...
		if (abs (_current_delta) >= sr) {
			...
			snprintf(buf, sizeof(buf), "\xCE\x94%s%s%d s", LEADINGZERO(abs(secs)), PLUSMINUS(-secs), abs(secs));
			buf[63] = '\0';
			return std::string(buf);
		}
	}
	/* left-align sign, to make it readable when decimals jitter */
	snprintf (buf, sizeof(buf), "\xCE\x94%s%s%lldsm", PLUSMINUS(-delta), LEADINGZERO(::llabs(delta)), ::llabs(delta));
	buf[63] = '\0';
	return std::string(buf);
}
